...
class ArduinoController:
    ...
    def _set_light_thread(self):
        ...
        try:
            light_stage = int(float(light) / 100 * 4095)
            self.send_command(f"i {light_stage}")
            logging.info(f"Light set to stage: {light_stage}")
        except ValueError:
            messagebox.showerror("Invalid Input", "Please enter a valid number for light power.")
            logging.warning("Invalid light power input.")
